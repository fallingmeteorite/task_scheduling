...
`@wait_branch_thread_ended`必须放在main_task上面,不能放到分支线程函数上面,防止主线程结束,分支线程还没运行完导致错误

`other_task`为需要运行的分支线程,上面必须添加`@branch_thread_control`装饰器来控制和监视和销毁
...
所有的分支线程都可以在网页端查看到运行状态(开启网页端请使用`start_task_status_ui()`),修改主线程的状态,分支线程将同步
,修改分支线程的状态将只会修改该分支线程的状态,该功能为统一状态

- 控制方式

这里提供两个控制函数:

`task_name`必须是唯一不重复的,在外部代码中用于获取其他分支线程的task_id(需要使用`_sharedtaskdict.read(task_name)`
获取task_id然后去终止，暂停或恢复)分支线程名字在网页控制端中将按照`main_task_name|task_name`显示,读取只需要填入`task_name`,
