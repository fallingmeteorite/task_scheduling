...
class TaskManager:
    ...
    def cancel_task(self, task_id: str) -> None:
        ...
        if self.check(task_id):
            ...
            try:
                self.tasks[task_id]['cancel'].cancel()
    ...
    def cancel_all_tasks(self) -> None:
        ...
        """
        self.is_operation_in_progress = True
        for task_id in list(self.tasks.keys()):  # Use list(self.tasks.keys()) to avoid errors caused by changing the size of the dictionary
    ...
    def terminate_task(self, task_id: str) -> None:
        ...
        if self.check(task_id):
            self.is_operation_in_progress = True
            try:
                self.tasks[task_id]['terminate'].join()
            except Exception as error:
                logger.error(error)
            finally:
                self.is_operation_in_progress = False
